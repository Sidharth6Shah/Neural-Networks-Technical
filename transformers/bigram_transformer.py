...
eval_iters = 200
n_embd = 384
n_head = 6 # Leads to each head outputting a 64D vector
n_layer = 6 # 6 blocks
dropout = 0.2
# ------------
...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        v = self.value(x) # Weighted aggregation of the values
        out = wei @ v
        return out
...
class BigramLanguageModel(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        # Get the embeddings of each token and embeddings of their locations in the block (both 32D)
        # Adding a language modelling head (lm_head) to turn the 32 vector input into a 65D vector (representative of vocab size, ie. each of the 65 possible characters)
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            # crop idx to the last block_size tokens because the pos_emb embedding table is size (8, 32), only supporting 8 tokens
